一款随机抽选APP，专为地偶DD而生  

### 特点
- 使用微信[小程序多端框架](https://developers.weixin.qq.com/miniprogram/dev/platform-capabilities/miniapp/intro/)开发构建
> 理论上可同步发行微信小程序、`Android`和`iOS`双端APP，因QQ小程序也兼容微信小程序代码，故也可发行QQ小程序。
- 首创“团体+成员”复合抽取，并支持指定单个团体抽取
- 编辑抽取名单自动缓存
- 支持深色模式

### 「正圆工坊」微信小程序（已下架）
~~微信搜一搜「正圆工坊」即可体验（2025年1月1日后）~~

### 「正圆工坊」APP（已全网下架）
- ~~2024年12月06日22时在[小米应用市场](http://app.xiaomi.com/detail/1622540)首发~~
- ~~2024年12月08日上架[应用宝](https://sj.qq.com/appdetail/org.zyworks.app)~~
- ~~2024年12月10日上架[华为应用市场](https://appgallery.huawei.com/app/C110623035)~~  
~~目前仅支持安卓，由于成本原因，暂不发布iOS版本，~~ 建议clone到本地，用自己的小程序appid和多端应用id构建使用  

### 说明
- 未来暂无增加新特性的计划    
- 本项目不涉及隐私信息收集，所有数据仅在本地缓存
